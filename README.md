...
machine is the virtual machine. The software or firmware that 
creates a virtual machine on the host hardware is called a 
...
```sh
 egrep '(vmx|svm)' /proc/cpuinfo #se l'output non è vuoto, 
...
```sh
 apt-get install kvm qemu-kvm libvirt-bin #per installarlo con 
  KVM
```
```sh
...
```sh
 qemu-img create -f qcow2 miaImg.img 10G #creiamo un'immagine 
  di tipo qcow2, di nome “miaImg.img” e di dimensione massima 
  pari a 10GB 
```
```sh
 qemu-img resize miaImg.img +10G #aumenta di 10GB la dimensione 
...
```sh
 qemu-system-x86_64 -enable-kvm -m 256 -hda miaImg.img -cdrom 
  nomeIso.iso -boot d #in questo caso stiamo inizializzando un 
...
```sh
 qemu-system-x86_64 -enable-kvm -m 256 -hda miaImg.img -cdrom 
  /dev/cdrom -boot d #in questo caso viene preso proprio il 
...
```sh
 qemu-system-x86_64 -enable-kvm -cpu host -smp 2 -hda miaImg.img 
  ...
  tablet -vga qxl -display sdl -monitor stdio #con smp 
...
```sh
 qemu-system-x86_64 -enable-kvm -cpu host -smp 2 -hda miaImg.img 
  ...
  qxl #per vga esistono diverse opzioni, se dovessimo avere 
...
```sh
 qemu-system-mips -cpu ? #in questo caso possiamo vedere i vari 
...
```sh
 qemu-system-arm -machine NameOfTheMachine -cpu ? #visualizza 
...
```sh
 qemu-system-x86_64 -enable-kvm -m 256 -hda miaImg.img 
  -kernel-kqemu #avvia il sistema operativo installato con qemu, 
  NOTA BENE, quest'istruzione prevede una CPU con 32 bit
```
```sh
 qemu-system-x86_64 -enable-kvm -m 256 -name debianProva -hda 
  miaImg.img #avvia il sistema operativo, e assegna un nome alla 
...
```
```sh
 qemu -m 256 -hda test01.img -kernel-kqemu & #lancia la nuova 
  macchina virtuale
```
...
```sh
 mount -o loop,offset=32356 /percorso/immagine.img 
  /mnt/mountpoint #monta l'immagine “RAW” su un mountpoint 
...
```sh
 Ctrl-a h # Get terminal commands:
```
```sh
 Ctrl-a h Ctrl-a ? # Print help 
```
```sh
...
```
```sh
 stop # Sospende l'esecuzione della macchina virtuale
```
```sh
 cont # Riprende l'esecuzione di una VM
```
...
```sh
 qemu-system-x86_64 -bios OVMF.fd -hda disk.img -cdrom 
  GNULinux.iso -boot d #esegue l'immagine di un sistema 
...
```sh
 qemu-system-x86_64 -soundhw ? #visualizza le schede audio 
  disponibili
```
...
```sh
 qemu-system-x86_64 -enable-kvm -m 2G -hda w7img.img -soundhw 
  hda #avvia l'immagine di un sistema operativo con una scheda 
...
```sh
 qemu-system-x86_64 -enable-kvm -m 2G -hda w7img.img -soundhw 
  hda -usb -device usb-host,hostbus=2,hostaddr=13 #esegue qemu 
...
```sh
 qemu.system-x86_64 -enable-kvm -hda miaImg.img -m 2G -smp 2 
  -redir tcp:5555::22 #in questo caso redirigiamo tutto il 
...
```sh
 ssh nomeAccountValidoGuest@localhost -p 5555 #in questo caso 
...
```sh
 ssh nomeAccountValidoHost@10.0.2.2 -p 22 #richiediamo una 
...
```sh
 route -n #visualizza l'indirizzo ip del sistema host sotto la 
  voce “Gateway”
```
### Qemu Redirection
...
```sh
 qemu-system-x86_64 -enable-kvm -hda VMs/debian.img -monitor 
  stdio #avvia una macchina virtuale, e apre il monitor di qemu 
...
```sh
 qemu-system-x86_64 -enable-kvm -m 2048 -smp 2 -hda VMs/lfs.img 
  -kernel /boot/vmlinuz-3.2.0-4-686-pae -append /boot/initrd.img 
  "root=/dev/sda2 console=tty0 console=ttyS0 rw" -serial 
  mon:stdio -nographic #l'opzione “-nographic” fa in modo di non 
...
```sh
 qemu-system-x86_64 -enable-kvm -m 2048 -smp 2 -hda 
  VMs/debian.img -serial mon:stdio -nographic #l'opzione “
...
library/java” or “library/nginx”, so in general starting with “
library/” are the official ones.
...
```sh
 An image: is a snapshot which does not change unless we do a 
  commit, in order to instantiate an image we can do a “docker 
  run”
```
```sh
...
```
```sh
 xhost + #it is an alternative to the previous command, 
...
```sh
 docker ps -a #view docker ID of the exited docker image we 
  want to save the state
```
from here we copy its short ID and do:

```sh
 docker commit 984d25f537c5 johnnyty/myapp:1.0 #here the code 
  is the short ID
```
now we can run 
...
```sh
 docker build -t myrepo/myapp:1.0 
  path/to/folder/containing/theDockerfile
```
let's see another example:

```sh
 docker build -t myrepo2/mywebapp:latest . #the build context 
...
```sh
 docker exec -i -t <containerID> /bin/bash #this opens another 
  shell on the container
```
In order to create a container we can do:

```sh
 docker create #creates a writeable container from the image 
  and prepares it for running.
```
```sh
 docker run #creates the container (same as docker create) and 
  runs it. 
```
...
```sh
 docker run -d tomcat:7 #this starts (and downloads if it 
...
```sh
 docker exec -it <containerID> /bin/bash #this starts a shell 
  on the container
```
...
```
```sh
 docker rmi -f myrepo/myapp:1.0 #in this case we delete the 
  image in a forced way
```
...
```sh
 docker push johnnytu/testimage:1.0 #this will begin the push 
  once we specify the credentials
```
...
```sh
 docker run -d -P -v /myvolume nginx:1.7 #in this case we mount 
  the dir /myvolume into the filesystem of the specified image
```
or another interesting example is:

```sh
 docker run -it -v /data/src:/test/src nginx:1.7 #in this case 
...
```sh
 docker run -d -p 8080:80 nginx:1.7 #maps port 80 on the 
  container to port 8080 on the host
```
instead if we would like to do the automatic mapping we do:

```sh
 docker run -d -P nginx:1.7 #this will do the automatic port 
...
```sh
 docker run -d -P --name website --link database:db nginx:1.7 
  #here, after --link we put the name of the source folowed by 
  its alias
```
let's see another example:
...
```sh
 docker logs containerName #it opens the log for the process ID 
  1 of that container
```
...
```sh
 docker run -d -P -v /nginxlogs:/var/log/nginx nginx #now we 
  can see logs in our directory /nginxlogs
```
...
```sh
 docker inspect containerName #this will display all the 
  details of the container in a JSON array
```
we can use grep to filter for a specific detail, for example:

```sh
 docker inspect containerName | grep IPAddress #this will show 
  the IP address of the specified container
```
...
```sh
 sudo service docker restart #riavvia il demone, è possibile 
...
```sh
 docker run -d -p 5000:5000 registry:2.0 #we must run an image 
...
```sh
 docker push myserver.net:5000/my-app:1.0 #where instead of 
...
## Removing all the intermediate layers by exporting a container 
...
```sh
 docker export my-running-image-id > ~/my_image.tar #the 
...
```sh
 lxc-checkconfig #mostra la lista di varie feature e mi dice se 
